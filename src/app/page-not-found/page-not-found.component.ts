...
export class StudentDetailComponent implements OnInit{
  ...
  student: Phone | undefined; //The student to display
  userList: Phone[] = [];// to store the list of students
  currentIndex: number = 0;//to track the current index

  constructor(
    private route: ActivatedRoute,
    private studentService: ContentService,
    private router: Router
  ) {}
//rewrite onInit to get the list of students and the current student
  ngOnInit(): void {
    this.studentService.getContents().subscribe(users => {
      this.userList = users;
...
//function to move backward through array with overflow protection
  goBackward(): void {
    if (this.currentIndex > 0) {
      this.currentIndex--;
      this.router.navigate(['/contents', this.userList[this.currentIndex].id]);
    }
  }
